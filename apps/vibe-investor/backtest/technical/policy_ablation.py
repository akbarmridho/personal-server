...
def _flag_codes(context: dict[str, Any], severity: str | None = None) -> set[str]:
    ...
    for raw_flag in flags:
        ...
        if severity is not None:
            if str(raw_flag.get("severity", "")).lower() != severity.lower():
                continue
        codes.add(code)
    return codes
...
def evaluate_flat_policy(
    ...
    if trend_bias == "bearish" or structure_status == "damaged":
        return PolicyDecision(
            action="WAIT",
            reason="daily_thesis_not_supportive",
            setup_id=setup_id,
            pending_setup_active=False,
        )
    ...
    if high_flags & ENTRY_BLOCKING_FLAGS:
        return PolicyDecision(
            action="WAIT",
            reason="high_severity_entry_blocker",
            setup_id=setup_id,
            pending_setup_active=False,
        )
    ...
    if trigger_state == "triggered" and risk_actionable:
        return PolicyDecision(
            action="BUY",
            reason="triggered_actionable_setup",
            setup_id=setup_id,
            pending_setup_active=False,
        )
    ...
    if setup.get("setup_validity") == "watchlist_only":
        return PolicyDecision(
            action="WAIT",
            reason="watchlist_setup_pending_trigger",
            setup_id=setup_id,
            pending_setup_active=True,
            pending_setup_id=setup_id,
            pending_setup_expiry_bars=stale_expiry_bars(setup_id),
        )

    return PolicyDecision(
        action="WAIT",
        reason="setup_not_actionable",
        setup_id=setup_id,
        pending_setup_active=False,
    )
...
def evaluate_long_policy(context: dict[str, Any]) -> PolicyDecision:
    ...
    if high_flags & EXIT_FLAGS:
        return PolicyDecision(
            action="EXIT",
            reason="high_severity_exit_flag",
            setup_id=setup_id,
            pending_setup_active=False,
        )
    ...
    if structure_status == "transitioning" and confirmation_state in {"mixed", "rejected"}:
        return PolicyDecision(
            action="EXIT",
            reason="transitioning_structure_with_weak_confirmation",
            setup_id=setup_id,
            pending_setup_active=False,
        )
