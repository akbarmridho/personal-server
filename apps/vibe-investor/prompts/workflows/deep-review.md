...
- Mandatory memory context: `memory/market/plan.md`, all other `.md` files in `memory/market/` (list and read), all files in `memory/notes/` (list and read), `get_state`, and the latest prior deep review if found. Surface any `get_state` warnings (staleness, status mismatches) in the synthesis output. The stale review hard gate from main.md applies here: print the full stale list before delegating symbol batches, include all stale `WATCHING`/`READY` symbols in the coverage universe, and include a "Stale Coverage" section in the synthesis output.
- Run `market-pulse` early in the workflow. Use the output to identify which symbols have active alerts, which are stale, and which screener/mover signals overlap with the coverage universe. Feed alert urgency into review prioritization.
- Additional top-down: portfolio behavior versus `IHSG` plus relevant leaders.
- Run order: `portfolio-management` first for holdings, realized and unrealized review, concentration/heat/cash-overlay checks, stale-plan detection, best-ideas-density review, and neglected-watchlist resurfacing; then delegated symbol or thesis review batches plus top-down market review in parallel via `task` tool; then parent synthesis. The market subagent (spawned via `task`) runs `technical-analysis` on IHSG and `narrative-analysis` at the market level, writes the IHSG Technical lens summary into `plan.md` and `narrative.md` to `memory/market/`, and returns a structured summary per `memory/market/README.md`.
- Before delegating symbol batches, check which symbols are missing artifacts. Subagents (spawned via `task`) must produce all required artifacts for every symbol they review. Deep-review is the primary opportunity to fill artifact gaps across the entire universe.
- Spawn all `task` calls (symbol batches + market subagent) in the same turn for parallel execution. Use the same subagent prompt templates as desk-check (see `desk-check.md` Phase 3) but adjust the mode and context for deep-review scope.
