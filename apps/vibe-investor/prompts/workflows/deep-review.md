...
- Mandatory memory context: `memory/market/plan.md`, all other `.md` files in `memory/market/` (list and read), all files in `memory/notes/` (list and read), `get_state`, and the latest prior deep review if found. Surface any `get_state` warnings (staleness, status mismatches) in the synthesis output.
