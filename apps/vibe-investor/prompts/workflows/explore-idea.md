...
Resolve the effective `TRADING_DAY` (most recent completed trading session) and `CALENDAR_DATE` (today in WIB) using the parent prompt's trading-day clock, then run `explore-idea`. Use `TRADING_DAY` for market data context. Use `CALENDAR_DATE` for all timestamps (`last_reviewed`, `last_updated`) and file naming.
