...
- During parent synthesis, report cumulative missed opportunity from `memory/notes/opportunity-cost.md` alongside `portfolio_heat`. Re-underwrite any READY symbol with >10% missed move or `wait_desk_check_count >= 3`. At `wait_desk_check_count >= 3`, the re-underwrite must produce a fresh `composite_decision` with current scores; if pilot gates pass, default outcome is PILOT entry. Renewal requires genuinely new evidence or materially different trigger levels — restating old evidence is not valid. At `wait_desk_check_count > 5`, hard expiry: PILOT if gates pass, or downgrade to WATCHING. No renewal.
