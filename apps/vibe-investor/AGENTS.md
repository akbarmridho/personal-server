...
- Base prompt should own only global agent behavior: identity, worldview, shared memory model, shared tool model, global workflow map, and subagent behavior.
- Command templates should stay thin. They should map user commands to workflows and pass minimal command-specific input, not own the full workflow contract.
