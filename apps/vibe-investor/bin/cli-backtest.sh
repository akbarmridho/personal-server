...
if [ -f .env ]; then
  export $(grep -v '^#' .env | grep -E 'OPENROUTER_API_KEY|OPENCODE_BACKTEST_CWD|OPENCODE_DATA_HOME|AI_CONNECTOR_DATA_ROOT|EXA_API_KEY' | xargs)
fi

# Validate required variables
missing=()
[ -z "$OPENCODE_BACKTEST_CWD" ] && missing+=("OPENCODE_BACKTEST_CWD")
...
WORK_DIR="$OPENCODE_BACKTEST_CWD"
