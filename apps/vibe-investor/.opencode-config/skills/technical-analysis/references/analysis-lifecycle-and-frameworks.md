...
```python
def classify_thesis_status(prior_invalidated: bool, structure_improved: bool, risk_worsened: bool):
    if prior_invalidated:
        return "invalidated"
    if structure_improved and not risk_worsened:
        return "improving"
    if risk_worsened:
        return "degrading"
    return "intact"
...
def normalize_mode(mode: str):
    allowed = {"INITIAL", "UPDATE", "THESIS_REVIEW", "POSTMORTEM"}
    if mode not in allowed:
        raise ValueError(f"Unsupported mode: {mode}")
    return mode


def normalize_lens(lens: str):
    allowed = {"UNIFIED", "CLASSICAL_TA", "WYCKOFF", "SMC_ICT_LIGHT"}
