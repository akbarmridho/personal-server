# Price Action Patterns And Breakouts

## Objective

Reference index for setup, breakout, and reversal modules.

## Module Index

- [Setup families and selection](setup-families-and-selection.md)
- [Breakout and participation](breakout-and-participation.md)
- [Reversal playbooks and confluence](reversal-playbooks-and-confluence.md)

## Usage Notes

- Select one setup family first.
- Validate breakout quality and participation before momentum calls.
- Require CHOCH plus confirmation BOS for reversal setups.
