...
export function buildSearchQuery(params: SearchStructuredParams): string {
  ...
  if (params.searchCriteria && params.searchCriteria.length > 0) {
    searchExpressions.push(
      ...buildUnifiedSearchExpressions(params.searchCriteria),
    );
  }
 ...
 */
function buildUnifiedSearchExpressions(
  searchCriteria: SearchCriteria[],
): string[] {
  ...
  for (let i = 0; i < searchCriteria.length; i++) {
    ...
    const effectiveLogic =
      i === searchCriteria.length - 1 ? undefined : criteria.logic;

    // If this is the first item in a group, or continuing the same logic
    if (
      currentGroup.length === 0 ||
      !effectiveLogic ||
      effectiveLogic === groupLogic
    ) {
  ...
  if (type === "relation") {
    ...
    if (!name.includes(".") && op !== "exists" && op !== "not_exists") {
      ...
      logVerboseTransform(
        ...
        name,
        enhancedName,
        "TriliumNext requires property access for relations",
      );
    }
  }
 ...
 */
function enhanceRelationProperty(
  name: string,
  op: string,
  value?: string,
): string {
...
function validateNoteType(value: string): string {
  ...
  if (!validNoteTypes.includes(value)) {
    throw new Error(
      `Invalid note type: '${value}'. Valid note types are: ${validNoteTypes.join(", ")}`,
    );
  }
...
function validateMimeType(value: string): string {
  ...
  if (!value.match(/^(text|application)\/[a-zA-Z0-9][a-zA-Z0-9\-_.]*$/)) {
    throw new Error(
      `Invalid MIME type format: '${value}'. Must follow pattern 'text/*' or 'application/*'`,
    );
  }
...
function validateISODate(value: string, property: string): string {
  ...
  const isValidSmartDate = smartDatePatterns.some((pattern) =>
    pattern.test(value.toUpperCase()),
  );

  if (!isValidISO && !isValidSmartDate) {
    throw new Error(
      `Invalid date format for property '${property}'. Must use ISO format: 'YYYY-MM-DDTHH:mm:ss.sssZ' (e.g., '2024-01-01T00:00:00.000Z') or TriliumNext smart expressions like 'TODAY-7', 'MONTH-1', 'YEAR+1'.`,
    );
  }
  ...
  if (isValidISO) {
    ...
    if (isNaN(dateObj.getTime())) {
      throw new Error(
        `Invalid date value for property '${property}': '${value}'. Please provide a valid ISO date.`,
      );
    }
  }
