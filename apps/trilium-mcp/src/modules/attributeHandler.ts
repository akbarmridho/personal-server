...
import {
  type Attribute,
  type AttributeOperationResult,
  type ManageAttributesParams,
  manage_attributes,
  type ReadAttributesParams,
  read_attributes,
...
export async function handleManageAttributes(
  ...
  try {
    ...
    if (
      ["create", "update", "delete"].includes(args.operation) &&
      args.attributes.length !== 1
    ) {
      return {
        content: [
          {
...
export async function handleReadAttributes(
  ...
  try {
    ...
    if (!permissionChecker.hasPermission("READ")) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        "Permission denied: Not authorized to read attributes.",
      );
    }
...
function format_read_attribute_response(
  ...
  if (result.success && result.attributes && result.attributes.length > 0) {
    ...
    const relations = result.attributes.filter(
...
function format_attribute_response(
  ...
  if (result.success && operation === "batch_create") {
    const templateRelations = result.attributes?.filter(
      (attr: Attribute) => attr.type === "relation" && attr.name === "template",
    );
