...
import {
  handleResolveNoteId,
  type ResolveNoteOperation,
} from "./resolveManager.js";

export async function handleResolveNoteRequest(
  args: any,
  ...
  if (response.requiresUserChoice && response.topMatches) {
    const choiceList = response.topMatches
      .map(
        (match, index) =>
          `${index + 1}. ${match.title} (ID: ${match.noteId}, Type: ${match.type}, Modified: ${match.dateModified})`,
      )
      .join("\n");
