...
export async function manage_attributes(
  ...
  try {
    switch (params.operation) {
      case "create":
        return await create_single_attribute(
          params.noteId,
          params.attributes[0],
          axiosInstance,
        );
      ...
      case "batch_create":
        return await create_batch_attributes(
          params.noteId,
          params.attributes,
          axiosInstance,
        );
...
async function create_batch_attributes(
  ...
  const promises = attributes.map(async (attribute) => {
    try {
      ...
      if (!validation.valid) {
        errors.push(
          `Validation failed for ${attribute.type} '${attribute.name}': ${validation.errors.join(", ")}`,
        );
        return null;
      }
...
async function update_attribute(
  ...
  try {
    ...
    logVerbose(
      "update_attribute",
      `Available attributes on note ${noteId}`,
      noteResponse.data.attributes,
    );

    // Find the attribute to update by name and type
    const targetAttribute = noteResponse.data.attributes.find(
      (attr: any) =>
        attr.name === attribute.name && attr.type === attribute.type,
    );

    if (!targetAttribute) {
      const availableAttrs = noteResponse.data.attributes
        .map((attr: any) => `${attr.type}:${attr.name}`)
        .join(", ");
      return {
        success: false,
    ...
    if (attribute.type === "label") {
      updateData.value =
        attribute.value !== undefined ? attribute.value : targetAttribute.value;
    }
    ...
    if (
      attribute.isInheritable !== undefined &&
      attribute.isInheritable !== targetAttribute.isInheritable
    ) {
      logVerbose(
        "update_attribute",
        "isInheritable change detected, requires delete+recreate",
        {
 ...
 */
function validate_attribute(attribute: Attribute): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (
    ...
    (typeof attribute.position !== "number" || attribute.position < 1)
  ) {
  ...
  if (
    attribute.isInheritable !== undefined &&
    typeof attribute.isInheritable !== "boolean"
  ) {
...
export async function read_attributes(
  ...
  try {
    ...
    const attributes: Attribute[] = response.data.attributes.map(
      (attr: any) => ({
        type: attr.type,
        name: attr.name,
        value: attr.value,
