...
export class FileManager {
  ...
  async createFileNote(
    ...
    try {
      ...
      throw new Error(
        `File data parsing failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
    ...
    if (
      params.mimeType &&
      !this.isCompatibleMimeType(params.mimeType, fileData.mimeType)
    ) {
      throw new Error(
        `MIME type mismatch: specified ${params.mimeType}, detected ${fileData.mimeType}`,
      );
    }
    ...
    if (fileData.size > (params.maxSize || this.maxFileSize)) {
      throw new Error(
        `File too large: ${(fileData.size / 1024 / 1024).toFixed(2)}MB. Maximum allowed: ${((params.maxSize || this.maxFileSize) / 1024 / 1024).toFixed(2)}MB`,
      );
    }
    ...
    try {
      ...
      const noteResponse = await this.axiosClient.post(
        "/create-note",
        noteData,
      );
      ...
      await this.uploadFileContentFromData(
        noteResult.note.noteId,
        fileData,
        mimeType,
      );
      ...
      throw new Error(
        `Failed to create file note: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  async uploadFileContentFromData(
    noteId: string,
    ...
    try {
      ...
      if (fileData.type === "path") {
        ...
        const buffer =
          fileData.data instanceof Buffer
            ? fileData.data
      ...
      if (axios.isAxiosError(error)) {
        ...
        const message = error.response?.data?.message || error.message;
        throw new Error(
          `Failed to upload file content (${status}): ${message}`,
        );
      }
      throw new Error(
  ...
  async getFileInfo(noteId: string): Promise<FileInfo> {
    try {
      ...
      if (note.type !== "file") {
        throw new Error(
          `Note ${noteId} is not a file note (type: ${note.type})`,
        );
      }
      ...
      try {
        ...
        const contentResponse = await this.axiosClient.get(
        ...
        const attachmentMatch = htmlContent.match(
          /src="api\/attachments\/([^"]+)\/[^"]+\/([^"]+)"/,
        );
        ...
        if (attachmentMatch) {
          ...
          const attachmentResponse = await this.axiosClient.get(
      ...
      return {
        ...
        fileUrl: attachmentInfo
          ? `api/attachments/${attachmentInfo.attachmentId}/document/${encodeURIComponent(note.title)}`
          : undefined,
  ...
  async processFileUpload(
    ...
    try {
      ...
      throw new Error(
        `File upload workflow failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private createReadableStreamFromBuffer(
    buffer: Buffer,
  ...
  private isCompatibleMimeType(specified: string, detected: string): boolean {
    ...
    return (
      compatibleTypes[specified]?.includes(detected) ||
