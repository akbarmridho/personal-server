...
import {
  extractTemplateRelation,
  getContentRequirements,
  validateContentForNoteType,
} from "../utils/contentRules.js";
import {
  logVerbose,
  logVerboseApi,
  logVerboseError,
} from "../utils/verboseUtils.js";
import type { SearchOperation } from "./searchManager.js";
...
function executeUnifiedSearch(
  ...
  try {
    ...
    throw new Error(
      `Invalid ${searchType} pattern: ${pattern}. Error: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function executeSearchReplace(
  ...
  try {
    ...
    throw new Error(
      `Search and replace failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
async function checkDuplicateTitleInDirectory(
  ...
  try {
    ...
    const response = await axiosInstance.get(
      `/notes?search=note.title%20%3D%20%27${encodeURIComponent(title)}%27%20AND%20note.parents.noteId%20%3D%20%27${encodeURIComponent(parentNoteId)}%27&fastSearch=false&includeArchivedNotes=true`,
    );
    const results = response.data.results || [];

    logVerbose(
      "checkDuplicateTitleInDirectory",
      `Search for duplicate title "${title}" in parent ${parentNoteId} found ${results.length} results`,
    );
...
export async function handleCreateNote(
  ...
  if (!parentNoteId || !title || !type) {
    throw new Error(
      "parentNoteId, title, and type are required for create operation.",
    );
  }
  ...
  if (!forceCreate) {
    const duplicateCheck = await checkDuplicateTitleInDirectory(
      parentNoteId,
      title,
      axiosInstance,
    );
    if (duplicateCheck.found) {
      return {
        ...
        choices: {
          ...
          createAnyway:
            "Create anyway - create duplicate note with same title (set forceCreate: true)",
          updateExisting:
            "Update existing - replace content of existing note with new content",
        },
  ...
  if (!contentValidation.valid) {
    return {
      ...
      duplicateFound: false,
      nextSteps:
        "Please adjust your content according to the requirements and try again.",
    };
  }
  ...
  if (attributes && attributes.length > 0) {
    try {
      logVerbose(
        "handleCreateNote",
        `Creating ${attributes.length} attributes for note ${noteId}`,
        attributes,
      );
      await createNoteAttributes(noteId, attributes, axiosInstance);
      logVerbose(
        "handleCreateNote",
        `Successfully created all attributes for note ${noteId}`,
      );
...
async function createNoteAttributes(
  ...
  const attributePromises = attributes.map(async (attr) => {
    ...
    logVerbose(
      ...
      `Created ${attr.type} '${attr.name}' for note ${noteId}`,
      response.data,
    );
    return response;
  });
  ...
  logVerbose(
    "createNoteAttributes",
    `Successfully created ${results.length} attributes for note ${noteId}`,
  );
}
...
export async function handleUpdateNote(
  ...
  if (!noteId || !expectedHash) {
    throw new Error(
      "noteId and expectedHash are required for update operation.",
    );
  }
  ...
  if (type !== "file" && !mode) {
    throw new Error(
      "mode is required for update operation. Please specify either 'overwrite' or 'append'.",
    );
  }
  ...
  if (type === "file" || type === "image") {
    ...
    if (fileUri) {
      ...
      try {
        ...
        await fileManager.uploadFileContentFromData(
          noteId,
          fileData,
      ...
      if (title) {
        ...
        try {
          ...
          throw new Error(
            `File metadata update failed: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      } else {
        throw new Error(
          "No changes specified for file note update. Provide title or fileUri.",
        );
      }
    }
  }
  ...
  try {
    ...
    try {
      ...
      logVerbose(
        "handleUpdateNote",
        "Could not read existing attributes for template validation",
        error,
      );
    }
    ...
    if (revision) {
      try {
        ...
        console.error(
          `Warning: Failed to create revision for note ${noteId}:`,
          error,
        );
    ...
    if (mode === "append") {
      ...
      const newProcessed = await processContentArray(
        finalContent,
      ...
      const processed = await processContentArray(
        finalContent,
        currentNote.data.type,
      );
      if (processed.error) {
      ...
      throw new Error(
        `Invalid mode: ${mode}. Mode must be either 'overwrite' or 'append'.`,
      );
    }

    const contentResponse = await axiosInstance.put(
    ...
    if (isMultiParamUpdate && title) {
      ...
      logVerboseApi("PATCH", `/notes/${noteId}`, patchData);
      const titleResponse = await axiosInstance.patch(
        `/notes/${noteId}`,
        patchData,
        {
          headers: {
      ...
      if (titleResponse.status !== 200) {
        throw new Error(
          `Unexpected response status for title update: ${titleResponse.status}`,
        );
      }
    }

    const revisionMsg = revisionCreated
      ? " (revision created)"
      : " (no revision)";
    const correctionMsg =
      finalContent !== rawContent ? " (content auto-corrected)" : "";
    const modeMsg =
      mode === "append" ? " (content appended)" : " (content overwritten)";
    const titleMsg =
      isMultiParamUpdate && title ? ` (title updated to "${title}")` : "";
...
export async function handleSearchReplaceNote(
  ...
  if (!searchPattern) {
    throw new Error(
      "searchPattern is required for search_and_replace operation.",
    );
  }

  if (!replacePattern) {
    throw new Error(
      "replacePattern is required for search_and_replace operation.",
    );
  }

  if (!expectedHash) {
    throw new Error(
      "expectedHash is required for search_and_replace operation. You must call get_note first to retrieve the current blobId.",
    );
  }
  ...
  try {
    ...
    const correctionMsg =
      finalContent !== newContent ? " (content auto-corrected)" : "";
    const revisionMsg = revisionCreated
      ? " (revision created)"
      : " (no revision)";
...
export async function handleGetNote(
  ...
  const isFileOrImageNote =
    noteData.type === "file" || noteData.type === "image";
  ...
  const { data: noteContent } = await axiosInstance.get(
    `/notes/${noteId}/content`,
    {
      responseType: "text",
    },
  ...
  if (searchPattern) {
    ...
    const matches = executeUnifiedSearch(
      searchContent,
      searchPattern,
      useRegex,
      searchFlags,
    ...
    const enhancedMatches = matches.map((match) => ({
      ...match,
      htmlContext: {
        contentType: (contentRequirements.requiresHtml ? "html" : "plain") as
          | "html"
          | "plain",
