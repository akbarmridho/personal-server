...
export async function handleCreateNoteRequest(
  ...
  if (!permissionChecker.hasPermission("WRITE")) {
    throw new McpError(
      ...
      "Permission denied: Not authorized to create notes.",
    );
  }
...
export async function handleUpdateNoteRequest(
  ...
  if (!permissionChecker.hasPermission("WRITE")) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      "Permission denied: Not authorized to update notes.",
    );
  }
  ...
  try {
    const currentNoteResponse = await axiosInstance.get(
      `/notes/${args.noteId}`,
    );
  ...
  if (args.fileUri) {
    ...
    const { parseFileDataSource, detectNoteTypeFromMime } = await import(
      "../utils/fileUtils.js"
    );
    try {
...
export async function handleDeleteNoteRequest(
  ...
  if (!permissionChecker.hasPermission("WRITE")) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      "Permission denied: Not authorized to delete notes.",
    );
  }
...
export async function handleGetNoteRequest(
  ...
  if (!permissionChecker.hasPermission("READ")) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      "Permission denied: Not authorized to get notes.",
    );
  }
...
export async function handleSearchReplaceNoteRequest(
  ...
  if (!permissionChecker.hasPermission("WRITE")) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      "Permission denied: Not authorized to modify notes.",
    );
  }
