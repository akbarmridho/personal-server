 ...
 * Process string content into ETAPI-compatible format
 */
export async function processContentArray(
  content: string,
  noteType?: string,
): Promise<ProcessedContent> {
  if (!content || content.trim() === "") {
    ...
    if (
      noteType &&
      ["book", "render", "search", "relationMap", "noteMap"].includes(noteType)
    ) {
      return { content: "" };
    }
 ...
 */
function validateContentForNoteType(
 ...
 */
function getContentRequirementsByNoteType(noteType: string): {
  format: "html" | "plain" | "base64" | "optional";
} {
  switch (noteType) {
    case "text":
    case "render":
