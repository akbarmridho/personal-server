...
export function detectMimeType(filePath: string): string {
  ...
  throw new Error(
    `Unsupported file type: ${ext}. Supported types: ${Object.values(SUPPORTED_MIME_TYPES).flat().join(", ")}`,
  );
}
...
function parseDataUri(dataUri: string): ParsedFileData {
  ...
  if (!match) {
    throw new Error(
      "Invalid data URI format. Expected: data:mime/type;base64,data",
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid base64 data in data URI: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
function parseBase64String(base64String: string): ParsedFileData {
  ...
  try {
    ...
    const mimeType =
      detectMimeTypeFromBuffer(buffer) || "application/octet-stream";
...
export function validateFilePath(filePath: string): ValidationResult {
  try {
    ...
    if (
      ...
      return {
        valid: false,
        error: "Invalid file path: path normalization failed",
      };
    }
 ...
 */
export function detectNoteTypeFromMime(
  mimeType: string,
): "image" | "file" | null {
 ...
 * Returns 'image' for images, 'file' for other supported file types, null for unsupported
 */
export function detectNoteTypeFromPath(
  filePath: string,
): "image" | "file" | null {
  try {
    const mimeType = detectMimeType(filePath);
