...
import type { Attribute, NoteType } from "../modules/noteManager.js";
  ...
  switch (noteType) {
    case "text":
      return {
        ...
        requiresHtml: true,
        description:
          "HTML content required (plain text auto-wrapped in <p> tags)",
        examples: ["<p>Hello world</p>", "<strong>Bold text</strong>"],
        errorMessage:
          "Text notes require HTML content. Plain text will be automatically wrapped in <p> tags.",
      };
    ...
    case "render":
      return {
        ...
        requiresHtml: false,
        description:
          "Content must be empty - render notes display HTML content from child notes via ~renderNote relation",
        examples: [""],
        errorMessage:
          "Render notes must be empty. Create a child code note with type='code' and mime='application/x-html' containing your HTML, then link it with ~renderNote='child-note-title' relation.",
      };
    ...
    case "code":
      return {
        ...
        errorMessage:
          "Code notes require plain text only. HTML tags are not allowed.",
      };
    ...
    case "mermaid":
      return {
        ...
        errorMessage:
          "Mermaid notes require plain text diagram syntax only. HTML tags are not allowed.",
      };
    ...
    case "webView":
      return {
        ...
        examples: [""],
        errorMessage:
          "WebView notes must be empty. Use the #webViewSrc label to specify the URL (e.g., #webViewSrc='https://example.com').",
      };
    ...
    case "search":
      return {
        ...
        requiresHtml: false,
        description:
          "Content must be empty - search queries are configured through the note's search properties",
        examples: [""],
        errorMessage:
          "Search notes must be empty. Search queries are configured through the note's search properties, not content.",
      };
    ...
    case "relationMap":
      return {
        ...
        requiresHtml: false,
        description:
          "Content must be empty - relation maps display note relationships visually",
        examples: [""],
        errorMessage:
          "RelationMap notes must be empty. The visual map is generated automatically from note relationships.",
      };
    ...
    case "noteMap":
      return {
        ...
        requiresHtml: false,
        description:
          "Content must be empty - note maps display note hierarchies visually",
        examples: [""],
        errorMessage:
          "NoteMap notes must be empty. The visual map is generated automatically from note hierarchies.",
      };
    ...
    case "book":
      return {
        ...
        requiresHtml: false,
        description:
          "Content must be empty - book notes are containers for child notes",
        examples: [""],
        errorMessage:
          "Book notes must be empty. They are container notes that organize child notes. Add content as child notes instead.",
      };
...
export function getTemplateContentRules(
  ...
  const templateValue =
    typeof templateRelation === "string"
      ? templateRelation
  ...
  if (
    noteType === "book" &&
    templateValue &&
    CONTAINER_TEMPLATES.includes(templateValue)
  ) {
    return {
      ...baseRules,
      allowContent: false,
 ...
 */
export function extractTemplateRelation(
...
export async function validateContentForNoteType(
  ...
  if (
    rules.allowedPatterns &&
    !rules.allowedPatterns.some((pattern) => pattern.test(textContent))
  ) {
    return {
      valid: false,
      content,
    ...
    case "mermaid":
      ...
      if (rules.requiresHtml === false && isLikelyHtml(textContent)) {
        return {
          ...
          content,
          error:
            `${noteType} notes require plain text only, but HTML content was detected. ` +
