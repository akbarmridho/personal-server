...
const handles = {
  golden: ["ApestorNyangkut", "TradingDiary2", "ArvinHonami"],
  rotation: [
    "pinkbourbon8898",
    "cheytax_1",
    "cukhurukuque",
...
export const getHandlesRandom = () => {
  const limit = 10;

  const remaining = limit - handles.golden.length;

  return [
    ...handles.golden,
    ...pickRandom(handles.rotation as any as string[], remaining),
  ];
};

export const searchRumourTargetted = async (params: {
  oldestDate?: string;
  newestDate?: string;
  handles: string[];
}) => {
...
- Focus on these handles: ${params.handles.join(", ")}
...
- Try to use other tools, like keyword or semantic search to explore more if existing result is insufficient. When use this approach, combine all target handles into a **single keyword or semantic search query** using OR operators (e.g., \`from:handle1 OR from:handle2 OR from:handle3\`) with \`limit: 100\` instead of making separate queries per account.
